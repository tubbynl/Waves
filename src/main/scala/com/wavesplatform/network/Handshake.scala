...
object Handshake extends ScorexLogging {
  def decode(in: ByteBuf): Handshake = {
    ...
    log.trace(s"==> Got app name size: $appNameSize")
    ...
    log.trace(s"==> Got app name: $appName")

    val appVersion = (in.readInt(), in.readInt(), in.readInt())
    log.trace(s"==> Got app version: $appVersion")

    val nodeNameSize = in.readByte()
    log.trace(s"==> Got node name size: $nodeNameSize")
    ...
    log.trace(s"==> Got node name: $nodeName")
  ...
  def decode2(ctx: ChannelHandlerContext, in: ByteBuf): Handshake = {
    val appNameSize = in.readByte()
    log.trace(s"${id(ctx)} ==> Got app name size: $appNameSize")

    require(appNameSize >= 0, s"Got an invalid application name's size: $appNameSize")
    val appName = in.readSlice(appNameSize).toString(Charsets.UTF_8)
    log.trace(s"${id(ctx)} ==> Got app name: $appName")

    val appVersion = (in.readInt(), in.readInt(), in.readInt())
    log.trace(s"${id(ctx)} ==> Got app version: $appVersion")

    val nodeNameSize = in.readByte()
    log.trace(s"${id(ctx)} ==> Got node name size: $nodeNameSize")
    require(nodeNameSize >= 0, s"Got an invalid node name's size: $nodeNameSize")
    val nodeName = in.readSlice(nodeNameSize).toString(Charsets.UTF_8)
    log.trace(s"${id(ctx)} ==> Got node name: $nodeName")
